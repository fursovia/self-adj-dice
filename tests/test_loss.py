...
def calculate_loss(logits: np.ndarray, targets: np.ndarray, gamma: float = 1.0) -> float:
    ...
    for curr_logits, curr_target in zip(logits, targets):
        ...
        prob_with_factor = (1 - curr_prob) * curr_prob
...
def test_numpy_vs_torch():
    criterion = SelfAdjDiceLoss(gamma=GAMMA)
    loss_torch = criterion(LOGITS_TORCH, TARGETS_TORCH).item()
    loss_numpy = calculate_loss(LOGITS_NUMPY, TARGETS_NUMPY, GAMMA)
